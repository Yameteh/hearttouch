...
public class ConversationAdapter extends CCPListAdapter <ECConversation> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(conversation != null) {
            if(TextUtils.isEmpty(nickName)) {
                ...
                mViewHolder.nickname_tv.setText(nickName);
            }
            ...
            if(conversation.getSessionId().toUpperCase().startsWith("G")) {
                ...
                mViewHolder.user_avatar.setImageResource(R.drawable.ic_unread_message);
    ...
    public void notifyChange() {
        ...
        if(cursor == null || cursor.getCount() < 1) {
            MatrixCursor matrixCursor = new MatrixCursor(new String[]{"unreadCount","type","sendStatus","dateTime","sessionId","text","username"},1);
            matrixCursor.addRow(new Object[]{1,1,3,System.currentTimeMillis(),account,mContext.getResources().getString(R.string.msg_empty_req),nickName});
            cursor = matrixCursor;
        }
        setCursor(cursor);
