...
//			}
	...
	private void setContactPhoto(Context context,BaseHolder baseHolder, ECMessage detail) {
		if (baseHolder.getChattingAvatar() != null) {
			if(detail.getDirection() == ECMessage.Direction.RECEIVE) {
				File bindHead = new File(context.getFilesDir(),"bindhead.png");
				baseHolder.getChattingAvatar().setImageURI(Uri.fromFile(bindHead));
			}else if(detail.getDirection() == ECMessage.Direction.SEND) {
				File saveHead = new File(context.getFilesDir(), "head.png");
				baseHolder.getChattingAvatar().setImageURI(Uri.fromFile(saveHead));
			}
		}
	}
