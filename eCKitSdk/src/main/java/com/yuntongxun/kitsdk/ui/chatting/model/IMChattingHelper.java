...
public class IMChattingHelper implements OnChatReceiveListener,
	...
	private OnMessageReportCallback mMessageReportCallback;
	...
	public static void setAccountInfo(String account,String nick) {
		getInstance().checkChatManager();
		final ECChatManager manager = getInstance().mChatManager;
		Log.d("YYY","setAccountInfo ");
		if(manager != null) {
			PersonInfo info = new PersonInfo();
			info.setUserId(account);
			info.setNickName(nick);
			Log.d("YYY","setAccountInfo "+info);
			manager.setPersonInfo(info, new ECChatManager.OnSetPersonInfoListener() {
				@Override
				public void onSetPersonInfoComplete(ECError ecError, int i) {
					Log.d("YYY","setAccountInfo "+ecError);
				}
			});
		}
	}
	...
	private class ChatManagerListener implements
		...
		public void onSendMessageComplete(ECError error, ECMessage message) {
			...
			if (message != null) {
				...
				if(mMessageReportCallback != null) {
					mMessageReportCallback.onMessageReport(error, message);
				}
				return;
			}
		}
	...
	public static void setOnMessageReportCallback(
		...
		getInstance().mMessageReportCallback = callback;
	}
	...
	private synchronized void postReceiveMessage(ECMessage msg,
		...
		Log.d("YYY","postReceiveMessage message");
		...
		boolean isHeartData = false;
		String msgUserData = msg.getUserData();
		Log.d("YYY","msgUserData "+msgUserData);
		if(!TextUtil.isEmpty(msgUserData) && msgUserData.startsWith("heartdata:")) {
			isHeartData = true;
		}
		if (!isHeartData && IMessageSqlManager
				.insertIMessage(msg, msg.getDirection().ordinal()) <= 0) {
			Log.d("YYY","insert message");
			return;
		}
		...
		if (mMessageReportCallback != null) {
			...
			msgs.add(msg);
			mMessageReportCallback.onPushMessage(msg.getSessionId(), msgs);
		}

		if(isHeartData){
			Intent intent = new Intent("com.binary.hearttouch.heartmessage");
			intent.putExtra("msg",msg);
			CCPAppManager.getContext().sendBroadcast(intent);
		}

		// 是否状态栏提示
		if (showNotice && !isHeartData)
	...
	private static void showNotification(ECMessage msg) {
		if (checkNeedNotification(msg.getSessionId())) {
			...
			//Add by yaoguoju
			String form = msg.getSessionId();
			ECNotificationManager.getInstance().showMsgNotifcation(form,lastMsg);
		...
		String currentChattingContactId = ECPreferences
				...
				.getString(
						...
						(String) ECPreferenceSettings.SETTING_CHATTING_CONTACTID
								.getDefaultValue());
		Log.d("YYY","contactid "+contactId+",cuurentChattingContactId "+currentChattingContactId);
		if (contactId == null) {
			return true;
		}
	...
	private synchronized void postDowloadMessageResult(ECMessage message) {
		...
		if(mMessageReportCallback != null) {
			mMessageReportCallback.onMessageReport(null,message);
		}
		...
		if (remove != null) {
			...
			if (mMessageReportCallback != null && remove.msg != null) {
				ArrayList<ECMessage> msgs = new ArrayList<ECMessage>();
				msgs.add(remove.msg);
				mMessageReportCallback.onPushMessage(
