...
public abstract class CCPActivityBase {
    ...
    public void init(Context context , FragmentActivity activity)  {
        ...
        if(getTitleLayout() != -1) {
            ...
            view.setBackgroundColor(context.getResources().getColor(R.color.red_btn_color_normal));
            if(Build.VERSION.SDK_INT >=19 && !filteredActivity()) {
                mRootView.addView(view, LinearLayout.LayoutParams.MATCH_PARENT,getStatusBarHeight(context));
            }
    ...
    private boolean filteredActivity() {
        String activity = getClassName();
        if("com.yuntongxun.kitsdk.ui.voip.VoIPCallActivity".equals(activity)) {
            return true;
        }
        return false;
    }
